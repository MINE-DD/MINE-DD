...
class Chat:
    def __init__(self, model_name, embeddings: Embeddings):
    ...
    def ask(self, question:str, docname:str, pages:list[int] = None):
        context = "\n".join(
            f"User: {q}\nAI: {a}" for q, a in self.history
        ) if self.history else "The conversation has just begun."

        try:
            document_chunks = self.embeddings.get_document_chunks(docname, pages)
            chunks_listed = "\n\n".join([f"Chunk {chunk.chunkname}) {chunk.text}" for chunk in document_chunks])
        except Exception as e:
            raise ValueError(f"Error retrieving document chunks for docname: {docname}, pages: {pages}. Error: {e}")
    ...
    def validate_docname_choice(self, user_input, available_docnames):
        valid_document_name = None
        if user_input.isdigit():
            index = int(user_input)
            if 0 <= index < len(available_docnames):
                valid_document_name = available_docnames[index]
                print(f"Selected document: {valid_document_name}")
        if user_input in available_docnames:
            valid_document_name = user_input
            print(f"\nSelected document: {valid_document_name}")
        return valid_document_name
    ...
    def chat_loop(self):
        ...
        available_docnames = list(embeddings.get_docs_details()["docname"])
        docnames_str_prompt = "\n".join(f"{i}) '{docname}'" for i, docname in enumerate(available_docnames))
        print(f"Available documents to ask questions:\n{docnames_str_prompt}")

        print("\nBefore beginning please tell me which document you are interested in (index number or name).")
        valid_document_name = None
        while valid_document_name is None:
            user_input = input("\nDocument: ")
            if user_input.lower() == "exit":
                print(f"{self.model_name}: Goodbye!")
                return
            else:
                valid_document_name = self.validate_docname_choice(user_input, available_docnames)
                if valid_document_name is None:
                    print(f"\nInvalid input. Please enter a number between 0 and {len(available_docnames) - 1} or a valid document name.\n{docnames_str_prompt}")
        ...
        while True:
            ...
            try:
                answer = self.ask(user_input, valid_document_name)
                print(f"{self.model_name} answer: {answer}\n")
            except Exception as e:
                print(f"Error: {e}")

if __name__ == "__main__":
    # Load MineDD Embeddings that the chat can use as context
    embeddings_dir = "../notebooks/outputs/my-embeddings.pkl"
    ...
    chat = Chat(model_name, embeddings)
    chat.chat_loop()
